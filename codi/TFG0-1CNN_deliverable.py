...
'''
'''
    STEP 1: Load the dataset (to modify based on path)
'''
...
model = kr.Sequential(
    [
        ...
        kr.layers.Conv1D(32, kernel_size=(3), activation="relu"),
        kr.layers.MaxPooling1D(pool_size=(2)),
        kr.layers.Conv1D(16, kernel_size=(3), activation="relu"),
        kr.layers.MaxPooling1D(pool_size=(2)),
        kr.layers.Flatten(),
        kr.layers.Dense(32, activation="relu", name="dense"),
        kr.layers.Dropout(0.5),
        kr.layers.Dense(16, activation="relu", name="dense2"),
        kr.layers.Dense(2, activation="softmax", name="dense3"),
    ]
)
...
pd.DataFrame(model.predict(x)).to_csv('./Results/predictions_1CNN.csv', index=False, header=False)
pd.DataFrame(y).to_csv('./Results/true_labels_1CNN.csv', index=False, header=False)
pd.DataFrame(history.history).to_csv('./Results/history_1CNN.csv', index=False)