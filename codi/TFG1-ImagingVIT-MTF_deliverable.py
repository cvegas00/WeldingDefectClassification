...
'''
    Obtain MTF
'''
X_mtf_voltage = MarkovTransitionField(n_bins=25).fit_transform(X_voltage)
x_mtf_voltage = MarkovTransitionField(n_bins=25).fit_transform(x_voltage)

X_mtf_current = MarkovTransitionField(n_bins=25).fit_transform(X_current)
x_mtf_current = MarkovTransitionField(n_bins=25).fit_transform(x_current)

'''
    Obtain All Images (Voltage)
'''
X_images = [X_mtf_voltage]
...
pd.DataFrame(model.predict(x_images)).to_csv('./Results/predictions_MTFVoltageViT.csv', index=False, header=False)
pd.DataFrame(y).to_csv('./Results/true_labels_MTFVoltageViT.csv', index=False, header=False)
pd.DataFrame(history.history).to_csv('./Results/history_MTFVoltageViT.csv', index=False)

'''
    Obtain All Images (Current)
'''
X_images = [X_mtf_current]
X_images = np.asarray(X_images)
X_images = np.reshape(X_images, (X_images.shape[1], X_images.shape[2], X_images.shape[3], X_images.shape[0]))
...
pd.DataFrame(model.predict(x_images)).to_csv('./Results/predictions_MTFCurrentViT.csv', index=False, header=False)
pd.DataFrame(y).to_csv('./Results/true_labels_MTFCurrentViT.csv', index=False, header=False)
pd.DataFrame(history.history).to_csv('./Results/history_MTFCurrentViT.csv', index=False)