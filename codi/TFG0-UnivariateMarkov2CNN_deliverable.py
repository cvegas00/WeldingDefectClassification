...
'''
n_states = 50
X_markov_voltage, _ = mk.get_MTM_images(pd.DataFrame(X_voltage), n_states, channels=1)
x_markov_voltage, test_states = mk.get_MTM_images(pd.DataFrame(x_voltage), n_states, channels=1)

X_markov_current, _ = mk.get_MTM_images(pd.DataFrame(X_current), n_states, channels=1)
x_markov_current, test_states = mk.get_MTM_images(pd.DataFrame(x_current), n_states, channels=1)

'''
    Obtain All Images (Voltage)
'''
X_images = [X_markov_voltage]           
...
pd.DataFrame(model.predict(x_images)).to_csv('./Results/predictions_markovVoltage2CNN.csv', index=False, header=False)
pd.DataFrame(y).to_csv('./Results/true_labels_markovVoltage2CNN.csv', index=False, header=False)
pd.DataFrame(history.history).to_csv('./Results/history_markovVoltage2CNN.csv', index=False)

'''
    Obtain All Images (Current)
'''
X_images = [X_markov_current]           
X_images = np.asarray(X_images)
print(X_images.shape)
X_images = np.reshape(X_images, (X_images.shape[1], X_images.shape[2], X_images.shape[3], X_images.shape[0]))
print(X_images.shape)
...
pd.DataFrame(model.predict(x_images)).to_csv('./Results/predictions_markovCurrent2CNN.csv', index=False, header=False)
pd.DataFrame(y).to_csv('./Results/true_labels_markovCurrent2CNN.csv', index=False, header=False)
pd.DataFrame(history.history).to_csv('./Results/history_markovCurrent2CNN.csv', index=False)